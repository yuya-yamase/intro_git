...
static U1 u1_s_poweric_cyc_regread_exe_flag;                    /* Power-IC I2c Cycle Check Register Read Excuting Flag */
...
void    vd_g_PowerIc_BonInit(void)
{
    ...
    u1_s_poweric_cyc_regread_exe_flag = (U1)FALSE;
...
static void vd_s_PowerIc_CycChk(void)
{
    ...
    && (Mcu_OnStep_PowerIc_OVRALL == (U1)MCU_ONSTEP_P_IC_OVERALL_FIN)){
        if(u1_s_poweric_cyc_regread_exe_flag == (U1)FALSE){         /* --Register Access Conflict Countermeasure-- */
    ...
    if(u1_s_poweric_i2ccycchk_stop_flag == (U1)FALSE){
        ...
        if(u1_t_time_chk_flg == (U1)TRUE){
            u1_s_poweric_cyc_regread_exe_flag = (U1)TRUE;           /* --Register Access Conflict Countermeasure-- */
            u1_t_func_sts = u1_s_PowerIc_I2cReadFunc();
            if(u1_t_func_sts == (U1)TRUE){
                u1_s_poweric_cyc_regread_exe_flag = (U1)FALSE;       /* --Register Access Conflict Countermeasure-- */
                /* Poling Timer Clear */
                u2_s_poweric_i2ccycchk_timer = (U2)0;
    ...
    switch (u1_s_poweric_turnondiag_sts){
        ...
        /* --------------- IB2 - IB7 Register Read --------------- */
            ...
            u1_t_reg_req_sts = u1_POWERIC_I2C_CTRL_REGREAD(&u2_s_poweric_regstep, &u4_s_poweric_i2c_ack_wait_time,
                                                           st_sp_POWERIC_TURNON_DATA_RD1_TBL, &u2_s_poweric_reg_btwn_time);
        ...
        /* --------------- MUTE ON Setting --------------- */
            /* Set Register */
            u1_t_reg_req_sts = u1_POWERIC_I2C_CTRL_REGSET(&u2_s_poweric_regstep, (U2)POWERIC_TURNON_MUTE_SET_WRINUM,
                                                          POWERIC_TURNON_MUTE_SET, &u4_s_poweric_i2c_ack_wait_time,
                                                          st_sp_POWERIC_TURNON_MUTE_SET_TBL, &u2_s_poweric_reg_btwn_time);
            if(u1_t_reg_req_sts == (U1)TRUE){
                /* Wait Timer Start */
        ...
        /* --------------- Amp OFF Setting --------------- */
                /* Set Register */
                u1_t_reg_req_sts = u1_POWERIC_I2C_CTRL_REGSET(&u2_s_poweric_regstep, (U2)POWERIC_TURNON_AMPONOFF_SET_WRINUM,
                                                              POWERIC_TURNON_AMPONOFF_SET, &u4_s_poweric_i2c_ack_wait_time,
                                                              st_sp_POWERIC_TURNON_AMPONOFF_SET_TBL, &u2_s_poweric_reg_btwn_time);
                if(u1_t_reg_req_sts == (U1)TRUE){
                    /* Wait Timer Clear */
                    u2_s_poweric_turnon_wait_timer = (U2)0;
                    /* Next Process */
        ...
        /* --------------- Diag Mode OFF Setting --------------- */
            /* Set Register */
            u1_t_reg_req_sts = u1_POWERIC_I2C_CTRL_REGSET(&u2_s_poweric_regstep, (U2)POWERIC_TURNON_DIAGMODE_SET_WRINUM,
                                                          POWERIC_TURNON_DIAGMODE_SET, &u4_s_poweric_i2c_ack_wait_time,
                                                          st_sp_POWERIC_TURNON_DIAGMODE_SET_TBL, &u2_s_poweric_reg_btwn_time);
            if(u1_t_reg_req_sts == (U1)TRUE){
                /* Wait Timer Start */
                u2_s_poweric_turnon_wait_timer = (U1)1;
                /* Next Process */
        ...
        /* --------------- DB0[0x90] Register Read --------------- */
                /* Read Register */
                u1_t_reg_req_sts = u1_POWERIC_I2C_CTRL_REGREAD(&u2_s_poweric_regstep, &u4_s_poweric_i2c_ack_wait_time,
                                                               st_sp_POWERIC_TURNON_DATA_RD2_TBL, &u2_s_poweric_reg_btwn_time);
                if(u1_t_reg_req_sts == (U1)TRUE){
                    /* Wait Timer Clear */
                    u2_s_poweric_turnon_wait_timer = (U2)0;
        ...
        /* --------------- DB1 - DB4 Register Read --------------- */
            /* Read Register */
            u1_t_reg_req_sts = u1_POWERIC_I2C_CTRL_REGREAD(&u2_s_poweric_regstep, &u4_s_poweric_i2c_ack_wait_time,
                                                           st_sp_POWERIC_TURNON_DATA_RD3_TBL, &u2_s_poweric_reg_btwn_time);
            if(u1_t_reg_req_sts == (U1)TRUE){
...
static U1   u1_s_PowerIc_I2cReadFunc(void)
{
    ...
    /* Read Register */
    u1_t_reg_req_sts = u1_POWERIC_I2C_CTRL_REGREAD(&u2_s_poweric_regstep, &u4_s_poweric_i2c_ack_wait_time,
                                                    st_sp_POWERIC_CYCLE_DATA_RD_TBL, &u2_s_poweric_reg_btwn_time);
    if(u1_t_reg_req_sts == (U1)TRUE){
        /* Function Completion */
        u1_t_ret = (U1)TRUE;
    }
