...
/****************************************************************************/
/* Object Name  | DCM/Dsp_SID27/CODE                                        */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
#if ( ( DCM_SEC_PECULIAR_TYPE_WHITE == STD_ON ) || ( DCM_SEC_PECULIAR_TYPE_BLACK == STD_ON ) )
#if ( DCM_SEC_PECULIAR_TYPE_WHITE == STD_ON )
#include <Dcm/Dcm_Dsp_StorageMng.h>
#endif /* DCM_SEC_PECULIAR_TYPE_WHITE == STD_ON */
...
#define DCM_DSP_SID27_DTMR_START_ATT_TH ((uint8)1U)                 /* Threshold of attempt counter to start delay timer */
#endif /* DCM_SEC_PECULIAR_TYPE_WHITE == STD_ON */
...
#if ( DCM_SEC_PECULIAR_TYPE_BLACK == STD_ON )
/****************************************************************************/
/* Function Name | Dcm_Dsp_SID27_Init                                       */
/* Description   | SID27 Initialization processing                          */
/* Preconditions | None                                                     */
...
#endif /* DCM_SEC_PECULIAR_TYPE_BLACK == STD_ON */
...
{
    if( ( u1ErrorCode == DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT )        /* NRC 0x13 */
     || ( u1ErrorCode == DCM_E_SUBFUNCTIONNOTSUPPORTED )                      /* NRC 0x12 */
     || ( u1ErrorCode == DCM_E_SUBFUNCTIONNOTSUPPORTEDINACTIVESESSION ) )     /* NRC 0x7E */
    {
        Dcm_Dsp_SID27_u1MngStatus = DCM_DSP_SID27_NO_SEEDREQUEST;
    }
...
)
{
    ...
    if( u1Result == (uint8)E_OK )
    {
        for( u1_Cnt = (uint8)0U; u1_Cnt < u1_SecurityNum; u1_Cnt++ )
        {
            if ( Dcm_Dsp_SID27_u1AttCntTmp[u1_Cnt] < DCM_DSP_SID27_DTMR_START_ATT_TH )
            {
                /* DelayTimer Stop */
                u1_TimerId = Dcm_Dsp_ConvertIdTbl[u1_Cnt].u1TimerId;
...
#define DCM_STOP_SEC_CODE
#include <Dcm_MemMap.h>
#endif /* DCM_SEC_PECULIAR_TYPE_WHITE == STD_ON || DCM_SEC_PECULIAR_TYPE_BLACK == STD_ON */
