/* can_rscf3_fs_phy_c_m10700_v2-2-0                                         */
/****************************************************************************/
/* Protected                                                                */
...
/****************************************************************************/
/* Object Name  | CAN/RSCF3/FS/PHYSICAL/CODE                                */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
#include <can/rscf3/can_rscf3_rx.h>
...
#define CAN_FS_u4PARAMCHK_CMNCFG_MASK   (0x1F7FFBFFUL)                      /* 通信パラメータチェック対象マスク(CmNCFG)  */
...
Can_fs_ResetInterrupt( uint8 u1Controller )
{
    ...
    uint32                           u4TxUseBit;    /* 使用送信MBOXビット                           */
    uint32                           u4TmpTxIntrBit;/* 割り込み送信MBOXビット                       */
    uint8                            u1Idx;         /* ループ用インデックス                         */

    /*------------------------------------------------------------------------------------------------*/
    /* 割り込み要因                     要求フラグ         許可ビット    要求フラグの使用／未使用     */
    /*------------------------------------------------------------------------------------------------*/
    /* CANm送信     送信完了            TMSTSp.TMTRF[1:0]  TMIECm.TMIEp  使用(使用MBのみ)             */
    /*              送信アボート        TMSTSp.TMTRF[1:0]  CmCTR.TAIE    使用(使用MBのみ)             */
    /*              送受信 FIFO送信完了 CFSTSk.CFTXIF      CFCCk.CFTXIE  未使用                       */
    ...
    /*                                  CmFDSTS.SOCO       CmCTR.SOCOIE  未使用                       */
    /*                                  CmFDSTS.EOCO       CmCTR.EOCOIE  未使用                       */
    /*                                  CmFDSTS.TDCVF      CmCTR.TDCVFIE 未使用                       */
    /*------------------------------------------------------------------------------------------------*/
    ...
    u4TxUseBit = ptTxMbBit->u4TxIntrBit | ptTxMbBit->u4TxPollBit;
    ...
    ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_0 ] = ptSetRSCANReg->u4CFSTS0Conf; /* 割り込み受信用FIFO:   送信は未使用、受信はコンフィグ依存 */
    ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_1 ] = CAN_u4CFSTSK_CLEARALL;       /* ポーリング受信用FIFO: 送受信共に割り込みは未使用         */
    ...
    *(ptChReg->ptu4CmFDSTS) = CAN_u4CMFDSTS_CLEARALL;
    ...
    u4TmpTxIntrBit = Can_tb_stTxMbBit[ u1Controller ].u4TxIntrBit;
    if ( ( *(ptChReg->ptu4TMIECm)                                             != u4TmpTxIntrBit )
    ...
    {   /*======== 割り込みマスク値異常時 ========*/
        ...
        /* CFSTSk.CFRXIF */
        ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_0 ] = CAN_u4CFSTSK_CLEARALL; /* 割り込み受信用FIFO */
...
Can_fs_DisableInterrupt( uint8 u1Controller )
{
    ...
    /* CFSTSk.CFTXIF, CFRXIF */
    ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_0 ] = CAN_u4CFSTSK_CLEARALL; /* 割り込み受信用FIFO   */
    ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_1 ] = CAN_u4CFSTSK_CLEARALL; /* ポーリング受信用FIFO */
    ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_2 ] = CAN_u4CFSTSK_CLEARALL; /* 未使用               */
...
Can_fs_ChkParameter( uint8 u1Controller )
{
    ...
    uint32                           u4CmNCFG;          /* CmNCFGレジスタラッチ用                       */
    uint32                           u4CmNCFGConfTmp;   /* CmNCFGレジスタコンフィグ値ラッチ用           */
...
{
    uint16 u2TargetPage;         /* 固着チェック対象ページ */
    uint16 u2StartPage;          /* 固着チェック開始ページ */
    uint16 u2ChkPage;            /* 固着チェックページ数   */
    uint16 u2LastPage;           /* 固着チェック最終ページ */
    uint16 u2Cnt;                /* ループ用カウンタ       */
...
/* Preconditions | -                                                        */
/* Parameters    | uint16 u2TargetPage   : RAMテスト対象ページ              */
...
Can_fs_RAMTestPage( uint16 u2TargetPage, uint8 u1ElmNum )
{
    ...
    /* RAMテスト対象ページ指定 */
    *(Can_tb_stGlobalReg.ptu4GTSTCFG) = (uint32)( ( (uint32)u2TargetPage << CAN_SFT_16 ) & CAN_u4GTSTCFG_RTMPS_MSK );
