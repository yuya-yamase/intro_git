/* can_rscf41_rx_phy_c_m10700_v2-2-1                                        */
/****************************************************************************/
/* Protected                                                                */
...
/****************************************************************************/
/* Object Name  | CAN/RSCF41/RX/PHYSICAL/CODE                               */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
#include <can/rscf41/can_rscf41_rx.h>
...
Can_rx_ChkRxFinMbox( uint8 u1Controller, uint8 u1Kind )
{
    ...
    ptChReg->ptu4CFSTS[ u1TRxFifoIdx ] = CAN_u4CFSTS_CFRXIF_CLR;

    u4CFSTS = ptChReg->ptu4CFSTS[ u1TRxFifoIdx ];

    u1MsgNum = (uint8)( ( u4CFSTS & CAN_u4CFSTS_CFMC_MSK ) >> CAN_SFT_8 );
    ...
    {
        u4CFID   = ptMb->u4IDReg;
        u4CFPTR  = ptMb->u4PTRReg;
        u4FDCSTS = ptMb->u4FDCSTSReg;

        if ( ( u4FDCSTS & CAN_u4CFFDCSTS_CFFDF_MSK ) == CAN_u4CFFDCSTS_CFFDF_CANFD )
        {
            ...
            ptChReg->ptu4CFPCTR[ u1TRxFifoIdx ] = CAN_u4CFPCTR_CFPC_SET;

            u4IdVal      = ( ( u4CFID & CAN_u4CFID_EID_MSK ) | CAN_RX_ID_FDF_CANFD );

            u1Dlc        = (uint8)( ( u4CFPTR & CAN_u4CFPTR_CFDLC_MSK ) >> CAN_SFT_28 );
        ...
        else
        {
            ...
            u4IdVal      = u4CFID & CAN_u4CFID_EID_MSK;

            u1DataLength = (uint8)( ( u4CFPTR & CAN_u4CFPTR_CFDLC_MSK ) >> CAN_SFT_28 );
        }

        stMsg.u4Id     = u4IdVal;
        stMsg.u1Length = u1DataLength;

        u1Mbox = (uint8)( ( u4FDCSTS & CAN_u4CFFDCSTS_CFPTR_MSK ) >> CAN_SFT_16 );
