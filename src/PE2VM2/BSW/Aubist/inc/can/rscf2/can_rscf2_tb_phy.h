/* can_rscf2_tb_phy_h_m10700_v3-0-0                                         */
/****************************************************************************/
/* Protected                                                                */
...
/****************************************************************************/
/* Object Name  | CAN/RSCF2/TB/PHYSICAL/HEADER                              */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/

#ifndef     CAN_RSCF2_TB_PHY_H
#define     CAN_RSCF2_TB_PHY_H
...
#define CAN_TB_TX_MB_DATA_ARYSIZE       (  5U )                 /* 送信用MB.DATA配列サイズ(20byte用) */
#define CAN_TB_TX_MB_DATA_MERGEARYSIZE  ( 16U )                 /* 送信用MB.DATA配列サイズ(64byte用) */
#define CAN_TB_RX_MB_DATA_ARYSIZE       ( 16U )                 /* 受信用MB.DATA配列サイズ(64byte用) */
#define CAN_TB_RX_MB_DATA_LIMITARYSIZE  (  5U )                 /* 受信用MB.DATA配列サイズ(20byte用) */
#endif /* (CAN_OPEMODE) */

#if (CAN_OPEMODE == CAN_OPEMODE_CANFD)
/* 送受信データ関連レジスタ型 予約領域 */
#define CAN_TB_MB_REG_RSV_TM            (  5U )                 /* 送信バッファデータ関連レジスタ型予約領域 004C-005F(4byte *  5) */
#define CAN_TB_MB_REG_RSV_FIFO          ( 13U )                 /* FIFOバッファデータ関連レジスタ型予約領域 004C-007F(4byte * 13) */
...
/*-------------------------------------------------*/
/* クラシカルCANモード：CmCFG レジスタ設定値       */
/* CAN FDモード       ：CmNCFGレジスタ設定値       */
...
/* クラシカルCANモード：CmCFG.  BRP[9:0] : fCAN[kHz] / CANクロック周波数[KHz] - 1 */
/* CAN FDモード       ：CmNCFG.NBRP[9:0] : fCAN[kHz] / CANクロック周波数[KHz] - 1 */
...
/* クラシカルCANモード：CmCFG.  TSEG1[3:0] : Prop_Seg + Phase_Seg1 - 1 */
/* CAN FDモード       ：CmNCFG.NTSEG1[6:0] : Prop_Seg + Phase_Seg1 - 1 */
/* ※ コンフィグ制約を設ける事で、割り切れないパターンはそもそも指定されないことを前提とする。*/
/*    ボーレートクロックが生成できない場合ダミー値 ALL 0を応答。                              */
...
/* クラシカルCANモード：CmCFG.  TSEG2[2:0] : Phase_Seg2 - 1 */
/* CAN FDモード       ：CmNCFG.NTSEG2[4:0] : Phase_Seg2 - 1 */
/* ※ コンフィグ制約を設ける事で、割り切れないパターンはそもそも指定されないことを前提とする。*/
/*    ボーレートクロックが生成できない場合ダミー値 ALL 0を応答。                              */
...
/* クラシカルCANモード：CmCFG.  SJW[1:0]   : SJW - 1 */
/* CAN FDモード       ：CmNCFG.NSJW[4:0]   : SJW - 1 */
/* ※ コンフィグ制約を設ける事で、割り切れないパターンはそもそも指定されないことを前提とする。*/
/*    ボーレートクロックが生成できない場合ダミー値 ALL 0を応答。                              */
...
#else /* (CAN_OPEMODE != CAN_OPEMODE_CAN) */
/* 各設定ビット位置 */
#define CAN_TB_CmCFG_SFT_SJW                ( 11U )
#define CAN_TB_CmCFG_SFT_TSEG2              ( 24U )
#define CAN_TB_CmCFG_SFT_TSEG1              ( 16U )
#define CAN_TB_CmCFG_SFT_BRP                (  0U )
/* 各設定ビットマスク */
#define CAN_TB_u4CmCFG_BIT_SJW              ( 0x0000F800UL )
#define CAN_TB_u4CmCFG_BIT_TSEG2            ( 0x1F000000UL )
#define CAN_TB_u4CmCFG_BIT_TSEG1            ( 0x007F0000UL )
#define CAN_TB_u4CmCFG_BIT_BRP              ( 0x000003FFUL )
#endif /* (CAN_OPEMODE) */

/* クラシカルCANモード：CmCFG レジスタ設定値算出              */
/* CAN FDモード       ：CmNCFGレジスタ設定値算出              */
/*   (参考)CmCFGレジスタ設定値例                              */
...
/*     CAN FDモード時                                         */
/*       0x030A1813 : CAN_TB_u4CmCFG(125K_2M_PTN1)            */
/*       0x040D180F : CAN_TB_u4CmCFG(125K_2P5M_PTN1)          */
/*       0x030A1809 : CAN_TB_u4CmCFG(250K_4M_PTN1)            */
/*       0x010C0009 : CAN_TB_u4CmCFG(250K_4M_PTN2)            */
/*       0x040D1807 : CAN_TB_u4CmCFG(250K_5M_PTN1)            */
/*       0x020F0007 : CAN_TB_u4CmCFG(250K_5M_PTN2)            */
/*       0x020B0804 : CAN_TB_u4CmCFG(500K_8M_PTN1)            */
/*       0x020B1004 : CAN_TB_u4CmCFG(500K_8M_PTN2)            */
/*       0x030A0804 : CAN_TB_u4CmCFG(500K_8M_PTN3)            */
/*       0x030A1804 : CAN_TB_u4CmCFG(500K_8M_PTN4)            */
/*       0x010C0004 : CAN_TB_u4CmCFG(500K_8M_PTN5)            */
/*       0x040D0803 : CAN_TB_u4CmCFG(500K_10M_PTN1)           */
/*       0x030E1003 : CAN_TB_u4CmCFG(500K_10M_PTN2)           */
/*       0x030E1803 : CAN_TB_u4CmCFG(500K_10M_PTN3)           */
/*       0x020F0803 : CAN_TB_u4CmCFG(500K_10M_PTN4)           */
/*       0x020F0003 : CAN_TB_u4CmCFG(500K_10M_PTN5)           */
/*       0x071E2001 : CAN_TB_u4CmCFG(500K_20M_PTN1)           */
/*       0xXXXXXXXX : CAN_TB_u4CmCFG(500K_32M_PTN1) 使用禁止  */
/*       0x0F3E3800 : CAN_TB_u4CmCFG(500K_40M_PTN1)           */
/*       0x0F3E7800 : CAN_TB_u4CmCFG(500K_40M_PTN2)           */
/*       0xXXXXXXXX : CAN_TB_u4CmCFG(500K_80M_PTN1) 使用禁止  */
...
#if (CAN_OPEMODE == CAN_OPEMODE_CAN)
    volatile uint32 u4DFReg[ CAN_TB_RX_MB_DATA_ARYSIZE ];       /* 0008-000F データフィールドレジスタ(クラシカルCANモード 8byte) */
#else /* (CAN_OPEMODE != CAN_OPEMODE_CAN) */
    volatile uint32 u4FDSTSReg;                                 /* 0008      CAN FDステータスレジスタ                          */
    volatile uint32 u4DFReg[ CAN_TB_RX_MB_DATA_LIMITARYSIZE ];  /* 000C-001F データフィールドレジスタ(CAN-FDモード 20byte)     */
#endif /* (CAN_OPEMODE) */
} CanTbRegCANRMType;
...
#endif /* (CAN_OPEMODE == CAN_OPEMODE_CANFD) */
    volatile uint32 u4DFReg [ CAN_TB_RX_MB_DATA_ARYSIZE ];      /* 0008-000F データフィールドレジスタ(クラシカルCANモード 8byte) */
                                                                /* 000C-004B データフィールドレジスタ(CAN-FDモード 64byte)       */
#if (CAN_OPEMODE == CAN_OPEMODE_CANFD)
    volatile uint32 u4RegPad[ CAN_TB_MB_REG_RSV_FIFO ];         /* 004C-007F Reserve                                           */
#endif /* (CAN_OPEMODE == CAN_OPEMODE_CANFD) */
} CanTbRegCANRFType;
...
#endif /* (CAN_OPEMODE == CAN_OPEMODE_CANFD) */
    volatile uint32 u4DFReg[ CAN_TB_TX_MB_DATA_ARYSIZE ];       /* 0008-000F データフィールドレジスタ(クラシカルCANモード 8byte) */
                                                                /* 000C-001F データフィールドレジスタ(CAN-FDモード 20byte)     */
} CanTbRegCANTMType;


#if (CAN_OPEMODE == CAN_OPEMODE_CANFD)
/* 送信バッファデータ関連レジスタ型(送信バッファマージモード用) */
typedef struct
{
    volatile uint32 u4IDReg;                                    /* 0000      IDレジスタ                                        */
    volatile uint32 u4PTRReg;                                   /* 0004      ポインタレジスタ                                  */
    volatile uint32 u4FDCTRReg;                                 /* 0008      CAN FDコンフィグレーションレジスタ                */
    volatile uint32 u4DFReg [ CAN_TB_TX_MB_DATA_MERGEARYSIZE ]; /* 000C-004B データフィールドレジスタ(CAN-FDモード 64byte)     */
    volatile uint32 u4RegPad[ CAN_TB_MB_REG_RSV_TM ];           /* 004C-005F Reserve                                           */
} CanTbRegCANTMMRGType;
#endif /* (CAN_OPEMODE == CAN_OPEMODE_CANFD) */
...
typedef struct
{
    ...
    volatile uint32 *                CanConst ptu4CmCFG;        /* クラシカルCANモード：                                       */
                                                                /*   チャネルコンフィグレーションレジスタ                      */
                                                                /* CAN FDモード       ：                                       */
                                                                /*   チャネル通常ビットレートコンフィグレーションレジスタ      */
...
#if (CAN_OPEMODE == CAN_OPEMODE_CANFD)
    volatile CanTbRegCANTMMRGType *  CanConst ptstTM_MERGE;     /* 送信バッファレジスタセット(送信バッファマージモード用)      */
#endif /* (CAN_OPEMODE == CAN_OPEMODE_CANFD) */
...
{
    /* Can Registers */
    volatile uint32 *                CanConst ptu4GRMCFG;       /* グローバルインタフェースモード選択レジスタ                  */
...
#if (CAN_OPEMODE == CAN_OPEMODE_CANFD)
    volatile uint32 *                CanConst ptu4GCRCCFG;      /* グローバルCRCコンフィグレーションレジスタ                   */
#endif /* (CAN_OPEMODE == CAN_OPEMODE_CANFD) */
    ...
    volatile uint32 *                CanConst ptu4CANFDMDR;     /* CAN-FDモードリードレジスタ                                  */
} CanTbGlobalRegType;
...
typedef struct
{
    ...
    uint32 u4CmCFGConf;                                         /* クラシカルCANモード：                                       */
    ...
    uint32 u4CFSTS1Conf;                                        /* 不正割り込み時用送受信FIFOバッファ1割り込みフラグクリア値   */
    uint32 u4CFSTS2Conf;                                        /* 不正割り込み時用送受信FIFOバッファ2割り込みフラグクリア値   */
    uint32 u4RFCCConf;                                          /* 受信FIFOバッファコンフィグレーションレジスタ設定値          */
#if (CAN_OPEMODE == CAN_OPEMODE_CANFD)
    ...
    uint32 u4CFFDCSTSkConf;                                     /* 送信要求時用CFESIビット設定値                               */
#endif /* (CAN_OPEMODE == CAN_OPEMODE_CANFD) */
...
extern CanConst uint8 Can_tb_u1TxFifo0LinkConf[ CAN_CFG_CONTROLLERNUM_MAX ];

/* 送信用FIFO1接続MBOX番号テーブル */
extern CanConst uint8 Can_tb_u1TxFifo1LinkConf[ CAN_CFG_CONTROLLERNUM_MAX ];

/* 送信用FIFO接続MBOXビットマスクテーブル */
extern CanConst uint16 Can_tb_u2TxFifoBitMask[ CAN_CFG_CONTROLLERNUM_MAX ];

/* 送信用FIFO書き込み回数の最大値テーブル */
extern CanConst uint8 Can_tb_u1FifoWriteSize[ CAN_CFG_CONTROLLERNUM_MAX ][ CAN_TRX_FIFO_FOR_TX ];

/* ポーリング受信用受信FIFO読み出し回数の最大値テーブル */
extern CanConst uint8 Can_tb_u1FifoReadSizePol[ CAN_CFG_CONTROLLERNUM_MAX ];

/* 割り込み受信用送受信FIFO 2 読み出し回数の最大値テーブル */
extern CanConst uint8 Can_tb_u1FifoReadSizeInt[ CAN_CFG_CONTROLLERNUM_MAX ];


#endif  /* CAN_RSCF2_TB_PHY_H */
