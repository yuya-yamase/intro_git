...
void vd_g_VISPwrInit(void)
{
    ...
    u1_s_vis_pwr_batvolt = (U1)VIS_PWR_BAT_FAIL;
...
static void vd_s_VISPwrGetBatVolt(void)
{
    U2 u2_t_bat1volt;        /* +B1電圧値(デジタル値) */
    U2 u2_t_bat2volt;        /* +B2電圧値(デジタル値) */
    ...
    if(u1_t_bat1ret == (U1)FALSE){
        /* Fail値を設定 */
        u2_t_bat1volt = (U2)VIS_PWR_BAT_FAIL;
    }
    
    /* +B2電圧値が正しく取得できなかった場合 */
    if(u1_t_bat2ret == (U1)FALSE){
        /* Fail値を設定 */
        u2_t_bat2volt = (U2)VIS_PWR_BAT_FAIL;
    }
...
static U1 u1_s_VISPwrBatVoltADCnv(const U2 u2_a_digitalbatvolt)
{
    ...
    u1_t_adcnvchk = (U1)VIS_PWR_ADCNVCHK_NOTOK;
    for(u1_t_idx = (U1)0U; u1_t_idx < (U1)VIS_PWRCFG_TABLENUM; u1_t_idx++){
        ...
        if(u2_a_digitalbatvolt <= st_gp_VIS_PWRCFG_ADTABLE[u1_t_idx].u2_batvoltmax){
            ...
            u1_t_adcnvchk = (U1)VIS_PWR_ADCNVCHK_OK;
            break;
        }
    }
    /* ADテーブル変換失敗時の処理 */
    if(u1_t_adcnvchk != (U1)VIS_PWR_ADCNVCHK_OK){
        u1_t_analogbatvolt = (U1)VIS_PWR_BAT_FAIL;
    }
...
/* ★SAILに実装予定の取得IF  */
/************************************************************************************************/
/* Function Name     : u1_g_VISPwrBatVoltGet(LSB:0.1)                                           */
/************************************************************************************************/
U1 u1_g_VISPwrBatVoltGet(void)
{
    return u1_s_vis_pwr_batvolt;
}
#endif
