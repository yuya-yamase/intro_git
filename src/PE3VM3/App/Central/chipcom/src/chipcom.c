...
void ChipCom_Main(void)
{
    ...
    if ((SpiState == XSPI_DCOND_IDLE) || (SpiState == XSPI_DCOND_TRANSMIT)) {
          ...
          for (TxLoopCnt = 0; TxLoopCnt < CHIPCOM_SND_FRM_MAX_WORD; TxLoopCnt++) {
            ChipCom_SpiTxDataU4[TxLoopCnt]  = (((uint32)ChipCom_SpiTxDataU1[(TxLoopCnt * 4)    ]                       ) & CHIPCOM_4BYTEMASK_LOWEST);
            ChipCom_SpiTxDataU4[TxLoopCnt] |= (((uint32)ChipCom_SpiTxDataU1[(TxLoopCnt * 4) + 1] << CHIPCOM_SHIFT_1BYTE) & CHIPCOM_4BYTEMASK_LOW);
            ChipCom_SpiTxDataU4[TxLoopCnt] |= (((uint32)ChipCom_SpiTxDataU1[(TxLoopCnt * 4) + 2] << CHIPCOM_SHIFT_2BYTE) & CHIPCOM_4BYTEMASK_HIGH);
            ChipCom_SpiTxDataU4[TxLoopCnt] |= (((uint32)ChipCom_SpiTxDataU1[(TxLoopCnt * 4) + 3] << CHIPCOM_SHIFT_3BYTE) & CHIPCOM_4BYTEMASK_HIGHEST);
          }
    ...
    if (SpiRead_Result == XSPI_OK)
    {
          ...
          for (RxLoopCnt = 0; RxLoopCnt < CHIPCOM_SND_FRM_MAX_WORD; RxLoopCnt++) {
            ChipCom_SpiRxDataU1[RxLoopCnt * 4]     = (uint8)(ChipCom_SpiRxDataU4[RxLoopCnt]  & CHIPCOM_4BYTEMASK_LOWEST);
            ChipCom_SpiRxDataU1[(RxLoopCnt * 4)+1] = (uint8)((ChipCom_SpiRxDataU4[RxLoopCnt] & CHIPCOM_4BYTEMASK_LOW)     >> CHIPCOM_SHIFT_1BYTE);
            ChipCom_SpiRxDataU1[(RxLoopCnt * 4)+2] = (uint8)((ChipCom_SpiRxDataU4[RxLoopCnt] & CHIPCOM_4BYTEMASK_HIGH)    >> CHIPCOM_SHIFT_2BYTE);
            ChipCom_SpiRxDataU1[(RxLoopCnt * 4)+3] = (uint8)((ChipCom_SpiRxDataU4[RxLoopCnt] & CHIPCOM_4BYTEMASK_HIGHEST) >> CHIPCOM_SHIFT_3BYTE);
          }
        ...
        if (data_id < CHIPCOM_DATAID_MAX) {
            ...
            if ((CHIPCOM_HEADER_LENGTH + transreq_len) <= (CHIPCOM_SND_FRM_MAX - ChipCom_SpiTxData_index)) {
                ...
                for (i = 0; i < transreq_len; i++) {
                    ChipCom_SpiTxDataU1[ChipCom_SpiTxData_index + i] = transreq_data[i];
                }
