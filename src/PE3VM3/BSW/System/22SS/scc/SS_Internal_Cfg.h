...
#define SS_USE_SLEEP      (STD_OFF) /* Use sleep */
#define SS_USE_MODE       (STD_ON)  /* Use mode */
#define SS_USE_SHUTDOWN   (STD_ON)  /* Use shutdown */
#define SS_USE_CORE_COUNT (1UL)     /* Use cores count */
...
#define SS_MM_USE_ASYNC_ARB        (STD_ON)                     /* 非同期状態遷移 */
...
/* SleepTm */
#define SS_SLEEPTM_u4SLEEPTIME_MAX (7200000UL) /* Sleep time max (LSB 1ms) */
...
#define SS_INTG_ASR_PKG (SS_INTG_AB)

#define SS_USE_UP (STD_ON)
