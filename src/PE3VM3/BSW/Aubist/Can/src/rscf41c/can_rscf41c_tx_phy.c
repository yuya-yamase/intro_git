/* can_rscf41c_tx_phy_c_m10700_v2-2-1                                       */
/****************************************************************************/
/* Protected                                                                */
...
/****************************************************************************/
/* Object Name  | CAN/RSCF41C/TX/PHYSICAL/CODE                              */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
Can_tx_TxReq_CanFrame( uint8 u1Controller, uint8 u1Mbox, CanConstR CanMsgType* ptMsg )
{
    ...
    u1TMSTS = ptChReg->ptu1TMSTS[ u1Mbox ];

    if ( ( u1TMSTS & CAN_u1TMSTS_REQ_RES_MSK ) == CAN_u1TMSTS_TXFREE )
    {
        u4IdVal = ptMsg->u4Id;
        if ( ( u4IdVal & CAN_u4BIT31 ) == CAN_u4BIT_NONE )
        {
            u4IdVal &= CAN_u4TMID_SID_MSK;
        }
        else
        {
            u4IdVal &= CAN_u4TMID_EID_MSK;
        }
    ...
    if ( Can_tb_stCtrlInfo[ u1Controller ].u1CtrlOpeMode == (uint8)CAN_OPEMODE_CANFD )
    {
        ...
        if( u1CheckData == (uint8)CAN_RAMCHK_VALUE_LOWERHALFBYTE )
        {
            ...
            if ( ( u1TMSTS & CAN_u1TMSTS_REQ_RES_MSK ) == CAN_u1TMSTS_TXFREE )
            {
                ...
                {
                    u4FrameInfoVal = CAN_u4TMFDCTR_FDF_CANFD | CAN_u4TMFDCTR_BRS_ON | Can_tb_stSetRSCANReg[ u1Controller ].u4TMFDCTRConf;
                }
                else
                {
                    u4FrameInfoVal = CAN_u4TMFDCTR_FDF_CANFD | CAN_u4TMFDCTR_BRS_OFF | Can_tb_stSetRSCANReg[ u1Controller ].u4TMFDCTRConf;
                }
...
Can_tx_TxAbt( uint8 u1Controller, uint8 u1Mbox )
{
    ...
    ptChReg->ptu1TMC[ u1Mbox ] = CAN_u1TMC_TMTAR_SET;

    u1TMSTS = ptChReg->ptu1TMSTS[ u1Mbox ];

    if ( ( u1TMSTS & CAN_u1TMSTS_REQ_RES_MSK ) == CAN_u1TMSTS_ABORTED )
    {
        ptChReg->ptu1TMSTS[ u1Mbox ] = CAN_u1TMSTS_TMTRF_CLR;
...
Can_tx_ChkTxFinMbox( uint8 u1Controller, uint8 u1Kind )
{
    ...
    u4TMTCSTSLo = ptChReg->ptu4TMTCSTS[CAN_IDX0];
    u4TMTCSTSHi = ptChReg->ptu4TMTCSTS[CAN_IDX1];

    u4TmpTxFinFlagLo = u4TMTCSTSLo & u4TxBitLo;
    u4TmpTxFinFlagHi = u4TMTCSTSHi & u4TxBitHi;
...
Can_tx_ChkTxAbtMbox( uint8 u1Controller )
{
    ...
    u4TMTASTSLo = ptChReg->ptu4TMTASTS[CAN_IDX0];
    u4TMTASTSHi = ptChReg->ptu4TMTASTS[CAN_IDX1];

    u4TmpTxAbtFlagLo = u4TMTASTSLo & u4TxBitLo;
    u4TmpTxAbtFlagHi = u4TMTASTSHi & u4TxBitHi;
...
Can_tx_SetTxReg( uint8 u1Controller, uint8 u1Mbox, uint8 u1DataWriteSize, CanConstR CanTxMsgInfoType* ptTxMsgInfo )
{
    ...
    ptChReg->ptu1TMC[ u1Mbox ] = CAN_u1TMC_TMTR_SET;
}
