/* can_rsc_st_phy_c_m10700_v2-2-0                                           */
/****************************************************************************/
/* Protected                                                                */
...
/****************************************************************************/
/* Object Name  | CAN/RSC/ST/PHYSICAL/CODE                                  */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
Can_st_StartChannel( uint8 u1Controller )
{
    ...
    if ( ( u2TMTRSTSy & u2TxMbUseBitMask ) == CAN_u2BIT_NONE )
    {
        ...
        /* ・Stop処理では、メッセージ送出中の強制中断によるエラーフレームを回避するため    */
        /*   まず既存送信をキャンセルし、キャンセル完了を待機してから、Stop処理をしている。*/
        /* ・一方、上位層は、キャンセル完了待ちで、Stop処理が未実施の段階であっても        */
        /*   Start要求を呼び出す可能性がある。                                             */
        /* ⇒ よって、本関数としては、送信要求が空であることを確認してから                 */
        /*    一旦チャネルリセットモードに遷移させ、Start時の設定を可能とする。            */
#endif /* (CAN_CFG_TRANSMIT == CAN_USE) */
    ...
    {   /* チャネルリセットモードの場合 */
        ...
        *(ptChReg->ptu4CmCTR)    = ptSetRSCANReg->u4CmCTRConf | CAN_u4CMCTR_CHMDC_RESET_SET;
        ...
        /* 送受信FIFO関連レジスタ */
        /* 送受信FIFOバッファステータスレジスタ(CFSTSk)のクリア */
        ...
        for ( u1MBox = (uint8)CAN_ST_BUF_SND_TOP; u1MBox < (uint8)CAN_NUM_BUF_SND_PER_CH; u1MBox++ )
        {
            /* 送信バッファ制御レジスタ(TMCp)のクリア          */
            /* 送信バッファステータスレジスタ(TMSTSp)のクリア  */
            /* → チャネルリセットモード遷移時初期化済み       */
        ...
        /* 送信完了割り込み許可設定 */
        /* - 送信バッファ割り込みイネーブルコンフィグレーションレジスタ(TMIECy) */
        *(ptChReg->ptu2TMIECy)  = Can_tb_stTxMbBit[ u1Controller ].u2TxIntrBit;

        /* 送信キュー関連レジスタ */
        /* - 送信キューコンフィグレーション/制御レジスタ(TXQCCm)のクリア */
        *(ptChReg->ptu4TXQCCm)  = CAN_u4TXQCCM_INIT;

        /* 送信キューステータスレジスタ(TXQSTSm)のクリア */
        /* →チャネルリセットモード遷移時初期化済み      */
