...
U1              u1_g_TripcomGetEMGF(U1 * u1p_a_emgf)
{
    ...
    u1_t_sts = u1_g_TripcomCfgGetEMGF(u1p_a_emgf);
...
U1              u1_g_TripcomGetPHVMDIND(U1 * u1p_a_phvmdind)
{
    ...
    u1_t_sts = u1_g_TripcomCfgGetPHVMDIND(u1p_a_phvmdind);
...
U1              u1_g_TripcomGetMaterialInfo(const U1 u1_a_INFOID, U2 * u2p_a_val)
{
    static  U1          (* const                fp_sp_TRIPCOM_FUELINFO[TRIPCOM_NUM_MTRLINFO])(U2 * u2p_a_val)           = {
        &u1_g_TripcomCfgFuelFull,
        &u1_g_TripcomCfgFuelVol
    };
    U1                                          u1_t_status;
...
void            vd_g_TripcomActRefuelEvHk(const U2 u2_a_DELTA, const U1 u1_a_VTM)
{
    U1          u1_t_id_fe;
    U1          u1_t_id_dte;
    U2          u2_t_ref_th[TRIPCOM_NUM_REFUEL_THR];
    U4          u4_t_loop;


    u1_t_id_fe  = (U1)TRIPCOM_REFUEL_THR_STP_FE  | (u1_a_VTM & (U1)TRUE);
    u1_t_id_dte = (U1)TRIPCOM_REFUEL_THR_STP_DTE | (u1_a_VTM & (U1)TRUE);
    
    for(u4_t_loop = (U4)0U ; u4_t_loop < (U4)TRIPCOM_NUM_REFUEL_THR ; u4_t_loop++){
        u2_t_ref_th[u4_t_loop] = (U2)0U;
    }
    vd_g_TripcomCfgRefuelThr(&u2_t_ref_th[0]);
    if (u2_a_DELTA >= u2_t_ref_th[u1_t_id_fe]) {
        u1_s_tripcom_refuel_actbit |= (U1)TRIPCOM_RSTRQ_ACT;
    }
    if (u2_a_DELTA >= u2_t_ref_th[u1_t_id_dte]) {
...
void            vd_g_TripcomGrphRstRq(const U2 u2_a_RSTRQBIT)
{
    u2_s_tripcom_grphreset |= u2_a_RSTRQBIT;
...
static  void    vd_s_TripcomCalcTask(const U2 u2_a_VEHSYSSTS, const U2 u2_a_VARBIT)
{
    ...
    for (u4_t_loop = (U4)0U; u4_t_loop < (U4)TRIPCOM_NUM_CNTTS; u4_t_loop++) {
        ...
        if (((stp_t_APPL->u2_varbit & u2_a_VARBIT) != (U2)0U) &&
            (u1_t_applid < (U1)TRIPCOM_NUM_APPL)) {

            stp_t_APPLIF = &st_gp_TRIPCOM_IF_CFG[u1_t_applid];
            if (stp_t_APPLIF->fp_vd_CONDCHK   != vdp_PTR_NA) {
                (stp_t_APPLIF->fp_vd_CONDCHK)(u1_t_cnttid, &u2_tp_stsfield[0]);
            }
            u1_t_update = (U1)FALSE;
            if (stp_t_APPLIF->fp_u1_CALCTRNST != vdp_PTR_NA) {
                u1_t_update = (stp_t_APPLIF->fp_u1_CALCTRNST)(u1_t_cnttid, &u2_tp_stsfield[0]);
            }
            if (stp_t_APPLIF->fp_vd_ACCMLT    != vdp_PTR_NA) {
                (stp_t_APPLIF->fp_vd_ACCMLT)(u1_t_cnttid, &u2_tp_stsfield[0], &u4_tp_delta[0]);
            }
