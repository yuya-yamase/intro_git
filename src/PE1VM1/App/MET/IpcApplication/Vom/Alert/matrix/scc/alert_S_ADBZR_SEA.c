...
/*  Version                                                                                                                          */
/*-----------------------------------------------------------------------------------------------------------------------------------*/
#define ALERT_S_ADBZR_SEA_C_MAJOR                      (5)
#define ALERT_S_ADBZR_SEA_C_MINOR                      (9)
#define ALERT_S_ADBZR_SEA_C_PATCH                      (1)
...
/*  Version Check                                                                                                                    */
/*-----------------------------------------------------------------------------------------------------------------------------------*/
#if (ALERT_S_ADBZR_SEA_C_MAJOR != ALERT_CFG_H_MAJOR)
#error "alert_S_ADBZR_SEA.c and alert_cfg_private.h : source and header files are inconsistent!"
#endif

/*-----------------------------------------------------------------------------------------------------------------------------------*/
/*  Literal Definitions                                                                                                              */
/*-----------------------------------------------------------------------------------------------------------------------------------*/
#define ALERT_S_ADBZR_SEA_NUM_DST                (64U)
...
/*-----------------------------------------------------------------------------------------------------------------------------------*/
static U4      u4_s_AlertS_adbzr_seaSrcchk(const U1 u1_a_VOM, const U4 u4_a_IGN_TM, const U1 u1_a_LAS);

/*-----------------------------------------------------------------------------------------------------------------------------------*/
/*  Constant Definitions                                                                                                             */
/*-----------------------------------------------------------------------------------------------------------------------------------*/
static const U1  u1_sp_ALERT_S_ADBZR_SEA_DST[ALERT_S_ADBZR_SEA_NUM_DST] = {
    ...
    (U1)ALERT_REQ_S_ADBZR_SEA_DOUBLE,                                          /* 33 DOUBLE                                          */
    (U1)ALERT_REQ_S_ADBZR_SEA_CYCLE,                                           /* 34 CYCLE                                           */
...
/*-----------------------------------------------------------------------------------------------------------------------------------*/
const ST_ALERT_MTRX st_gp_ALERT_S_ADBZR_SEA_MTRX[1] = {
    {
        &u4_s_AlertS_adbzr_seaSrcchk,                                          /* fp_u4_SRC_CHK                                      */
        vdp_PTR_NA,                                                            /* fp_vd_XDST                                         */

        (const U4 *)vdp_PTR_NA,                                                /* u4p_MASK                                           */
        (const U4 *)vdp_PTR_NA,                                                /* u4p_CRIT                                           */

        &u1_sp_ALERT_S_ADBZR_SEA_DST[0],                                       /* u1p_DST                                            */
        (U2)ALERT_S_ADBZR_SEA_NUM_DST,                                         /* u2_num_srch                                        */
...
static U4      u4_s_AlertS_adbzr_seaSrcchk(const U1 u1_a_VOM, const U4 u4_a_IGN_TM, const U1 u1_a_LAS)
{
    static const U2 u2_s_ALERT_S_ADBZR_SEA_TO_THRESH = ((U2)5000U / (U2)OXCAN_MAIN_TICK);
    static const U1 u1_s_ALERT_S_ADBZR_SEA_LSB_MSG   = (U1)3U;
    static const U4 u4_s_ALERT_S_ADBZR_SEA_BAT_WT    = (U4)0x00000020U;
    U4              u4_t_src_chk;
    U1              u1_t_msgsts;
    U1              u1_t_sgnl;
    ...
    u1_t_msgsts   = u1_g_oXCANRxdStat((U2)OXCAN_RXD_PDU_CAN_BSR1S01_CH0,
                                      ...
                                      u2_s_ALERT_S_ADBZR_SEA_TO_THRESH) & ((U1)COM_TIMEOUT | (U1)COM_NO_RX);

    u4_t_src_chk  = (U4)u1_t_sgnl;
    u4_t_src_chk |= ((U4)u1_t_msgsts << u1_s_ALERT_S_ADBZR_SEA_LSB_MSG);

    if((u1_a_VOM & (U1)ALERT_VOM_BAT_WT) != (U1)0U){
        u4_t_src_chk |= u4_s_ALERT_S_ADBZR_SEA_BAT_WT;
    }
...
/*  5.9.1    12/17/2025  EA       Change for BEV System_Consideration_2.(MET-S_ADBZR-CSTD-0-06-A-C0)                                 */
/*                                                                                                                                   */
