...
/*  Version                                                                                                                          */
/*-----------------------------------------------------------------------------------------------------------------------------------*/
#define ALERT_B_ALOA2_C_MAJOR                    (5)
#define ALERT_B_ALOA2_C_MINOR                    (0)
#define ALERT_B_ALOA2_C_PATCH                    (0)
...
/*  Version Check                                                                                                                    */
/*-----------------------------------------------------------------------------------------------------------------------------------*/
#if (ALERT_B_ALOA2_C_MAJOR != ALERT_CFG_H_MAJOR)
#error "alert_B_ALOA2.c and alert_cfg_private.h : source and header files are inconsistent!"
#endif

/*-----------------------------------------------------------------------------------------------------------------------------------*/
/*  Literal Definitions                                                                                                              */
/*-----------------------------------------------------------------------------------------------------------------------------------*/
#define ALERT_B_ALOA2_NUM_DST                    (64U)
...
/*-----------------------------------------------------------------------------------------------------------------------------------*/
static U1      u1_s_alert_b_aloa2_zn11S19_sts;
...
/*-----------------------------------------------------------------------------------------------------------------------------------*/
static const U1  u1_sp_ALERT_B_ALOA2_DST[ALERT_B_ALOA2_NUM_DST] = {
    ...
    (U1)ALERT_REQ_B_ALOA2_RUN_LT,                                              /* 33 RUN_LT                                          */
    (U1)ALERT_REQ_B_ALOA2_PARK_LT,                                             /* 34 PARK_LT                                         */
...
/*-----------------------------------------------------------------------------------------------------------------------------------*/
const ST_ALERT_MTRX st_gp_ALERT_B_ALOA2_MTRX[1] = {
    {
        &u4_s_AlertB_aloa2Srcchk,                                              /* fp_u4_SRC_CHK                                      */
        vdp_PTR_NA,                                                            /* fp_vd_XDST                                         */

        (const U4 *)vdp_PTR_NA,                                                /* u4p_MASK                                           */
        (const U4 *)vdp_PTR_NA,                                                /* u4p_CRIT                                           */

        &u1_sp_ALERT_B_ALOA2_DST[0],                                           /* u1p_DST                                            */
        (U2)ALERT_B_ALOA2_NUM_DST,                                             /* u2_num_srch                                        */
...
void    vd_g_AlertB_aloa2Init(void)
{
    u1_s_alert_b_aloa2_zn11S19_sts = (U1)COM_NO_RX;
}
...
static U4      u4_s_AlertB_aloa2Srcchk(const U1 u1_a_VOM, const U4 u4_a_IGN_TM, const U1 u1_a_LAS)
{
    ...
    u1_t_msgsts   = u1_g_oXCANRxStat((U2)OXCAN_PDU_RX_CAN_ZN11S19_RXCH0,
    ...
    u1_t_sgnl     = (U1)0U;
    (void)Com_ReceiveSignal(ComConf_ComSignal_LTOPE_AD, &u1_t_sgnl);
    u4_t_src_chk |= (U4)u1_t_sgnl;

    if((u1_a_VOM & (U1)ALERT_VOM_BAT_WT) != (U1)0U){
        u4_t_src_chk |= u4_s_ALERT_B_ALOA2_BIT_BAT_WT;
    }
...
/* --------- ----------  -------  -------------------------------------------------------------------------------------------------- */
/*  5.0.0     2/23/2024  TR       Newly created for 19PFv3.                                                                          */
/*                                                                                                                                   */
/*  * TR   = Tebs Ramos, DTPH                                                                                                        */
/*                                                                                                                                   */
/*                                                                                                                                   */
