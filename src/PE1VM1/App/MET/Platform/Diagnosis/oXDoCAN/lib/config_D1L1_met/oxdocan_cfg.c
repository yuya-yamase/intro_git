...
#define OXDC_SUPPORT_SES_NUM                     (4U)
...
/*-----------------------------------------------------------------------------------------------------------------------------------*/
const U2                u2_g_OXDC_RPG_TIM_TO_RUN = (U2)1000U / (U2)10U;
...
void    vd_g_oXDoCANCfgMainStart(const ST_OXDC_REQ * st_ap_REQ, const U2 u2_a_TSLOT)
{
    ...
    vd_g_TyDoCANDilMainTask(st_ap_REQ->u1_eom_aft);
}
/*===================================================================================================================================*/
...
void    vd_g_oXDoCANCfgServiceMain(const ST_OXDC_REQ * st_ap_REQ, ST_OXDC_ANS * st_ap_ans, const U2 u2_a_TSLOT)
{
    ...
    vd_g_TyDoCANDtcMaMainTask(st_ap_REQ->u1_eom_aft);
#endif /* #if ((defined(BSW_BSWM_CS_CFG_FUNC_CS)  ) && ... */
    ...
    /* ---------------------------------------------------------------------------------------------------- */
     ...
     (BSW_BSWM_CS_CFG_FUNC_CS   ==BSW_USE) && \
     (BSW_BSWM_CS_CFG_FUNC_SECOC==BSW_USE))
    IdsM_Ab_MainFunctionDem();
#endif  /* #if ((defined(BSW_BSWM_CS_CFG_FUNC_CS)  ) && ... */
    vd_g_oXDoCANRoutCfgMain();
}
/*===================================================================================================================================*/
...
void    vd_g_oXDoCANCfgRpgSwReset(void)
{
    vd_g_StartupFblEntrySetMark((U4)STARTUP_FBL_ENTRY_BY_DGTOOL);
    vd_s_oXdcPrepSwReset();
    vd_g_EcuMTriggerSwReset((U1)ECU_M_SWRST_TYPE_SOFT, (U1)ECU_M_SWRST_BY_RPG_RQST);
}
#endif
/*===================================================================================================================================*/
...
void    vd_g_oXDoCANCfgRpgCancel(void)
{
    vd_g_EcuMTriggerSwReset((U1)ECU_M_SWRST_TYPE_SOFT, (U1)ECU_M_SWRST_BY_DG_RQST);
}
/*===================================================================================================================================*/
...
U2      u2_g_oXDoCANCfgKmph(void)
{
    ...
    u1_t_rx_stat = u1_g_oXCANRxStat((U2)OXCAN_PDU_RX_CAN_VSC1G13,
                                    (U2)OXCAN_RX_SYS_NRX_POE | (U2)OXCAN_RX_SYS_TOE_VDC,
        ...
        for(u1_t_mem = (U1)0U; u1_t_mem < u1_g_OXDC_RBA_MEM_NUM_CFG; u1_t_mem++){
            ...
            if((u4_t_mem_begin <  u4_t_mem_end  ) &&
               ...
               (u4_t_src_nbyte >  (U4)0U        )){
                ...
                u4_t_dst_end   = u4_t_dst_begin + u4_t_src_nbyte;
                if((st_ap_ANS->u4_nbyte              >= u4_t_src_nbyte                  ) &&
                   ((u4_t_src_begin >= u4_t_dst_end) || (u4_t_dst_begin >= u4_t_src_end))){
...
static void    vd_s_oXdcPrepSwReset(void)
{
    volatile U4 *                  u4_tp_raddr;
    volatile U4                    u4_t_rdbk;

    u4_tp_raddr = (volatile U4 *)OXDC_MR_RA_WORD_RESFC;
    vd_REG_U4_WRITE(*u4_tp_raddr, (U4)OXDC_MR_REA_INIT);

    u4_tp_raddr = (volatile U4 *)OXDC_MR_RA_WORD_RESF;
    u4_t_rdbk = u4_REG_READ(*u4_tp_raddr);

    u4_tp_raddr = (volatile U4 *)OXDC_MR_RA_WORD_RESFCR;
    vd_REG_U4_WRITE(*u4_tp_raddr, (U4)OXDC_MR_REA_INIT);

    u4_tp_raddr = (volatile U4 *)OXDC_MR_RA_WORD_RESFR;
    u4_t_rdbk = u4_REG_READ(*u4_tp_raddr);
