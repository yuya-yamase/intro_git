...
/* バージョン情報 */
/* Version Number */
#define CRI_ATOM_AT_VERSION				(0x00590000)
#define CRI_ATOM_AT_VER_NUM				".59.00"
 ...
 */
#define CRIATOMAWB_WORKSIZE_FOR_LOADFROMMEMORY	(8 * (CriSint32)sizeof(CriUintPtr))
...
/*JP
 * \brief RawPCM Floatボイスプール作成用コンフィグ構造体にデフォルト値をセット
 * \ingroup ATOMEXLIB_AT
 * \param[out]	p_config	RawPCM Floatボイスプール作成用コンフィグ構造体へのポインタ
 * \par 説明:
 * ::criAtomExVoicePool_AllocateRawPcmFloatVoicePool_AT 関数に設定するコンフィグ構造体
 * （ ::CriAtomExRawPcmFloatVoicePoolConfig_AT ）に、デフォルトの値をセットします。<br>
 * \sa CriAtomExRawPcmFloatVoicePoolConfig_AT, criAtomExVoicePool_AllocateRawPcmFloatVoicePool_AT
 */
#define criAtomExVoicePool_SetDefaultConfigForRawPcmFloatVoicePool_AT(p_config)	\
{																				\
	(p_config)->identifier = 0;													\
	(p_config)->num_voices = CRIATOMEX_DEFAULT_VOICES_PER_POOL;					\
	(p_config)->player_config.reserved = 0;										\
 ...
 * HCAプレーヤ作成用コンフィグ構造体
 * \par 説明:
 * HCAが再生可能なプレーヤを作成する際に、動作仕様を指定するための構造体です。<br>
...
typedef struct CriAtomHcaPlayerConfigTag {
	/*JP
		...
		::criAtomPlayer_CreateHcaPlayer 関数で作成されたAtomプレーヤは、max_channelsで指定した
	...
	*/
	...
	/*JP
		...
		::criAtomPlayer_CreateHcaPlayer 関数で作成されたAtomプレーヤは、max_sampling_rateで指定した
...
typedef struct CriAtomSoundGeneratorPlayerConfigTag {
 ...
 /*JP
 * \brief RawPCM Floatプレーヤ用コンフィグ構造体
 * \ingroup ATOMLIB_AT
 * \par 説明:
 * RawPCM Floatプレーヤを作成する際に指定するパラメータです。<br>
 * ::criAtomPlayer_CalculateWorkSizeForRawPcmFloatPlayer_AT 関数および
 * ::criAtomPlayer_CreateRawPcmFloatPlayer_AT 関数の config として指定します。
 * \par 備考:
 * デフォルト設定を使用する場合、 ::criAtomPlayer_SetDefaultConfigForRawPcmFloatPlayer_AT マクロで
 * 構造体にデフォルトパラメータをセットした後、::criAtomPlayer_CalculateWorkSizeForRawPcmFloatPlayer_AT 関数
 * または ::criAtomPlayer_CreateRawPcmFloatPlayer_AT 関数に構造体を指定してください。<br>
 * \attention
 * 将来的にメンバが増える可能性があるため、 ::criAtomPlayer_SetDefaultConfigForRawPcmFloatPlayer_AT
 * マクロを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。<br>
 * （構造体のメンバに不定値が入らないようご注意ください。）
 * \sa criAtomPlayer_CalculateWorkSizeForRawPcmFloatPlayer_AT, criAtomPlayer_CreateRawPcmFloatPlayer_AT
 */
typedef struct CriAtomRawPcmFloatPlayerConfigTag_AT {
	CriSint32	reserved;
} CriAtomRawPcmFloatPlayerConfig_AT;
...
/*JP
 * \brief RawPCM Floatボイスプール作成用コンフィグ構造体
 * \ingroup ATOMEXLIB_AT
 * \par 説明:
 * RawPCM Floatボイスプールの仕様を指定するための構造体です。<br>
 * ::criAtomExVoicePool_AllocateRawPcmFloatVoicePool_AT 関数に引数として渡します。<br>
 * \attention
 * 将来的にメンバが増える可能性があるため、 ::criAtomExVoicePool_SetDefaultConfigForRawPcmFloatVoicePool_AT
 * マクロを使用しない場合には、使用前に必ず構造体をゼロクリアしてください。<br>
 * （構造体のメンバに不定値が入らないようご注意ください。）
 * \sa criAtomExVoicePool_AllocateRawPcmFloatVoicePool_AT, criAtomExVoicePool_SetDefaultConfigForRawPcmFloatVoicePool_AT
 */
typedef struct CriAtomExRawPcmFloatVoicePoolConfigTag_AT {
	CriAtomExVoicePoolIdentifier		identifier;		/*JP< ボイスプール識別子	*/
	CriSint32							num_voices;		/*JP< プールするボイスの数	*/
	CriAtomRawPcmFloatPlayerConfig_AT	player_config;	/*JP< ボイスの仕様			*/
} CriAtomExRawPcmFloatVoicePoolConfig_AT;
 ...
 * \param[out]		data			PCMデータのチャンネル配列
 * \return							消費済みサンプル数
 * \par 説明:
 * AtomのPCM出力結果を受け取ることが出来るコールバック関数です。<br>
 * <br>
 * コールバック関数の登録には ::criAtomEx_SetPcmMixerOutputCallback 関数を使用します。<br>
 * コールバック関数を登録すると、PCMミキサーがミックスを行った後に、
 * コールバック関数が実行されるようになります。<br>
 * <br>
 * PCM出力コールバック関数には、 PCM データのフォーマットやチャンネル数、
 ...
 * （インターリーブされていません。）<br>
 * 第 5 引数（ data 配列）には、各チャンネルの PCM データ配列の先頭アドレスが格納されています。<br>
 * （二次元配列の先頭アドレスではなく、チャンネルごとの PCM データ配列の先頭アドレスを格納した
 * 一次元のポインタ配列です。）<br>
 * 格納されてくる PCM データはPCMミキサーにてミックスされた後の音声です。<br>
 * <br>
 * プラットフォームによって、 PCM データのフォーマットは異なります。<br>
 * 実行環境のデータフォーマットについては、第 2 引数（ format ）で判別可能です。<br>
 ...
 * .
 * <br>
 * 戻り値には消費済みサンプル数を指定して下さい。<br>
 * 消費出来なかったサンプルは次回コールバック時に追加分と合わせて再度出力されます。<br>
 * なお、0を戻り値に指定した場合、次回コールバック時には再度同じバッファが出力されます。<br>
 * \attention
 ...
 * <br>
 * 戻り値の消費済みサンプル数に対して負値やnum_samplesを超える値を設定した場合、
 * 出力サンプルを全て消化したものと見なして次回出力が行われます。<br>
 * \sa criAtomEx_SetPcmOutputCallback_AT
 */
typedef CriSint32 (*CriAtomExPcmOutputCbFunc_AT)(void *obj, CriAtomPcmFormat format, 
												CriSint32 num_channels, CriSint32 num_samples, void *data[]);

/*JP
 * \brief PCMデータの出力準備完了コールバック関数
 * \ingroup ATOMEXLIB_GLOBAL
 * \par 説明:
 * \param[in]		obj				ユーザ指定オブジェクト
 * \return							なし
 * \par 説明:
 * AtomがPCMデータの出力を開始するタイミングに実行されるコールバック関数です。<br>
 * <br>
 * コールバック関数の登録には ::criAtomEx_SetPcmOutputStartCallback_AT 関数を使用します。<br>
 * コールバック関数を登録すると、 ::criAtomEx_Initialize_AT 関数による初期化済みの状態で、
 *  ::criAtomEx_ExecuteMain 関数の初回実行時に実行されます。
 * \sa criAtomEx_SetPcmOutputStartCallback_AT
 */
typedef void (*CriAtomExPcmOutputStartCbFunc_AT)(void *obj);

/*JP
 * \brief PCMデータの出力終了コールバック関数
 * \ingroup ATOMEXLIB_GLOBAL
 * \par 説明:
 * \param[in]		obj				ユーザ指定オブジェクト
 * \return							なし
 * \par 説明:
 * AtomがPCMデータの出力を停止するタイミングに実行されるコールバック関数です。<br>
 * <br>
 * コールバック関数の登録には ::criAtomEx_SetPcmOutputStopCallback_AT 関数を使用します。<br>
 * コールバック関数を登録すると、 ::criAtomEx_Fialize_AT 関数実行時にて、コールバック関数が実行されます。
 * \sa criAtomEx_SetPcmOutputStopCallback_AT
 */
typedef void (*CriAtomExPcmOutputStopCbFunc_AT)(void *obj);
 ...
 * <br>
 * AWBハンドルは内部的にバインダ（ CriFsBinderHn ）を確保します。<br>
 * AWBファイルのTOC情報をロードする場合、AWBハンドル数分のバインダが確保できる設定で
 ...
 * \return		CriSint32	ワーク領域サイズ
 * \par 説明:
 * ::criAtomExAcb_LoadAcbDataWithAwbHn 関数の実行に必要なワーク領域サイズを計算します。
 * ::criAtomEx_SetUserAllocator マクロによるアロケータ登録を行わずに
 * ::criAtomExAcb_LoadAcbDataWithAwbHn 関数でAWBデータをロードする際には、
 * 本関数が返すサイズ分のメモリをワーク領域として渡す必要があります。<br>
 * <br>
 * ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。<br>
 * ワーク領域サイズの計算に失敗した理由については、エラーコールバックのメッセージで確認可能です。<br>
 ...
 * に指定したパラメータによって変化します。<br>
 * そのため、本関数を実行する前に、ライブラリを初期化しておく必要があります。<br>
 * \sa criAtomExAcb_LoadAcbDataWithAwbHn
 */
CriSint32 CRIAPI criAtomExAcb_CalculateWorkSizeForLoadAcbDataWithAwbHn(
	void *acb_data, CriSint32 acb_data_size, CriAtomAwbHn awb_hn);
 ...
 * ::criAtomExAcb_CalculateWorkSizeForLoadAcbDataWithAwbHn 関数で計算します。<br>
 * <br>
 ...
 * \attention
 * 本関数を実行する前に、ライブラリを初期化しておく必要があります。<br>
 * \sa criAtomExAcb_CalculateWorkSizeForLoadAcbDataWithAwbHn, CriAtomExAcbHn, criAtomExPlayer_SetCueId
 */
CriAtomExAcbHn CRIAPI criAtomExAcb_LoadAcbDataWithAwbHn(
	void *acb_data, CriSint32 acb_data_size, CriAtomAwbHn awb_hn,
 ...
 * AtomExプレーヤでHCAデータ（もしくはHCAデータを含むキュー）の再生を行うと、
 * AtomExプレーヤは作成されたHCAボイスプールからボイスを取得し、再生を行います。<br>
 * <br>
 * ボイスプールの作成に成功すると、戻り値としてボイスプールハンドルが返されます。<br>
 * アプリケーション終了時には、作成したボイスプールを ::criAtomExVoicePool_Free 
 * 関数で破棄する必要があります。<br>
 ...
 * （ステレオHCAデータを再生する場合、AtomExプレーヤは、ステレオHCAが再生可能な
 * ボイスプールからのみボイスを取得します。）<br>
 * ただし、ステレオのボイスプールを作成した場合、モノラルデータ再生時にステレオ
 * ボイスプールのボイスが使用される可能性はあります。<br>
 * <br>
...
/*JP
 * \brief RawPCM Floatボイスプール作成用ワーク領域サイズの計算
 * \ingroup ATOMEXLIB_AT
 * \param[in]	config		RawPCM Floatボイスプール作成用コンフィグ構造体
 * \return		CriSint32	ワーク領域サイズ
 * \retval		0以上		正常に処理が完了
 * \retval		-1			エラーが発生
 * \par 説明:
 * RawPCM Floatボイスプールの作成に必要なワーク領域のサイズを計算します。<br>
 * ::criAtomEx_SetUserAllocator マクロによるアロケータ登録を行わずに
 * ::criAtomExVoicePool_AllocateRawPcmFloatVoicePool_AT 関数でボイスプールを作成する際には、
 * ::criAtomExVoicePool_AllocateRawPcmFloatVoicePool_AT 関数に本関数が返すサイズ分のメモリをワーク
 * 領域として渡す必要があります。<br>
 * <br>
 * ワーク領域サイズの計算に失敗すると、本関数は -1 を返します。<br>
 * ワーク領域サイズの計算に失敗した理由については、エラーコールバックのメッセージで確認可能です。<br>
 * \par 備考:
 * ボイスプールの作成に必要なワークメモリのサイズは、プレーヤ作成用コンフィグ
 * 構造体（ ::CriAtomExRawPcmFloatVoicePoolConfig_AT ）の内容によって変化します。<br>
 * <br>
 * 引数にNULLを指定した場合、デフォルト設定
 * （ ::criAtomExVoicePool_SetDefaultConfigForRawPcmFloatVoicePool_AT マクロ使用時
 * と同じパラメータ）でワーク領域サイズを計算します。
 * <br>
 * 引数 config の情報は、関数内でのみ参照されます。<br>
 * 関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても
 ...
 * に指定したパラメータによって変化します。<br>
 * そのため、本関数を実行する前に、ライブラリを初期化しておく必要があります。<br>
 * \sa criAtomExVoicePool_AllocateRawPcmFloatVoicePool_AT
 */
CriSint32 CRIAPI criAtomExVoicePool_CalculateWorkSizeForRawPcmFloatVoicePool_AT(
	const CriAtomExRawPcmFloatVoicePoolConfig_AT *config);
 ...
 * \return		CriAtomExVoicePoolHn	ボイスプールハンドル
 * \par 説明：
 * RawPCM Floatボイスプールを作成します。<br>
 * （RawPCM Floatボイスは、波形生成を行うボイスです。）<br>
 * <br>
 * ボイスプールを作成する際には、ワーク領域としてメモリを渡す必要があります。<br>
 * 必要なメモリのサイズは、 ::criAtomExVoicePool_CalculateWorkSizeForRawPcmFloatVoicePool_AT 
 * 関数で計算します。<br>
 * （::criAtomEx_SetUserAllocator マクロを使用してアロケータを登録済みの場合、
 * 本関数にワーク領域を指定する必要はありません。）<br>
 * <br>
 * 本関数を実行することで、RawPCM Floatの再生が可能なボイスがプールされます。<br>
 * AtomExプレーヤで再生を行うと、AtomExプレーヤは作成されたRawPCM Floatボイスプールから
 * ボイスを取得し、再生を行います。<br>
 * <br>
 * ボイスプールの作成に成功すると、戻り値としてボイスプールハンドルが返されます。<br>
 * アプリケーション終了時には、作成したボイスプールを ::criAtomExVoicePool_Free 
 ...
 * \par 備考:
 * ボイスプール作成時には、プール作成用コンフィグ構造体
 * （ ::CriAtomExRawPcmFloatVoicePoolConfig_AT 構造体の num_voices ）
 * で指定した数分のボイスが、ライブラリ内で作成されます。<br>
 * 作成するボイスの数が多いほど、同時に再生可能な音声の数は増えますが、
 * 反面、使用するメモリは増加します。<br>
 * <br>
 ...
 * ボイスプールの作成／破棄は、シーンの切り替わり等、負荷変動を許容できる
 * タイミングで行うようお願いいたします。<br>
 * \sa CriAtomExRawPcmFloatVoicePoolConfig_AT, criAtomExVoicePool_CalculateWorkSizeForRawPcmFloatVoicePool_AT,
 * criAtomExVoicePool_Free
 */
CriAtomExVoicePoolHn CRIAPI criAtomExVoicePool_AllocateRawPcmFloatVoicePool_AT(
	const CriAtomExRawPcmFloatVoicePoolConfig_AT *config, void *work, CriSint32 work_size);
 ...
 * \return		CriSint32	ワーク領域サイズ
 * \par 説明:
 * ライブラリを初期化するために必要なワーク領域のサイズを取得します。<br>
 * \par 備考:
 * ライブラリが必要とするワーク領域のサイズは、ライブラリ初期化用コンフィグ
 * 構造体（ ::CriAtomExConfig_AT ）の内容によって変化します。<br>
 * <br>
 * 引数 config の情報は、関数内でのみ参照されます。<br>
 * 関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても
 * 問題ありません。
 * \attention
 * 本関数は ::criAtomEx_Initialize_AT 関数を使用して
 * ライブラリを初期化する場合に使用します。<br>
 * ::criAtomEx_Initialize 関数を使用する場合には、本関数ではなく
 * ::criAtomEx_CalculateWorkSize 関数を使用してワーク領域サイズを計算してください。<br>
 * \sa CriAtomExConfig_AT, criAtomEx_Initialize_AT
 */
CriSint32 CRIAPI criAtomEx_CalculateWorkSize_AT(const CriAtomExConfig_AT *config);
 ...
 * ユーザは ::criAtomEx_SetPcmOutputCallback_AT 関数を使用してコールバック関数
 * （ ::CriAtomExPcmOutputCbFunc_AT ）をセットし、ライブラリから定期的にPCMデータを
 * 取得してアプリケーション側で音声出力を行う必要があります。<br>
 * \attention
 ...
 * 	.
 * \sa CriAtomExConfig_AT, criAtomEx_Finalize_AT,
 * criAtomEx_SetPcmOutputCallback_AT, CriAtomExPcmOutputCbFunc_AT
 */
void CRIAPI criAtomEx_Initialize_AT(
	const CriAtomExConfig_AT *config, void *work, CriSint32 work_size);
 ...
 * ライブラリを終了します。<br>
 * \attention
 * 本関数は ::criAtomEx_Initialize_AT
 * 関数を使用してライブラリを初期化した場合に使用します。<br>
 * ::criAtomEx_Initialize 関数を使用した場合には、本関数ではなく
 * ::criAtomEx_Finalize 関数を使用して終了処理を行ってください。<br>
 * \sa criAtomEx_Initialize_AT
 */
void CRIAPI criAtomEx_Finalize_AT(void);

/*JP
 * \brief PCM出力コールバック関数の登録
 * \ingroup ATOMEXLIB_AT
 * \param[in]	cb_func		PCM出力のコールバック関数
 * \param[in]	obj			ユーザ指定オブジェクト
 * \par 説明:
 * PCMの出力結果を受け取るコールバック関数を登録します。<br>
 * 登録されたコールバック関数は、AtomがミックスしたPCMが出力されるタイミングに
 * 呼び出されます。<br>
 * \attention
 ...
 * エラーが発生したり、デッドロックが発生する可能性があります。<br>
 * <br>
 * PCM出力コールバック関数内で長時間処理をブロックすると、
 * 音切れ等の問題が発生しますので、ご注意ください。<br>
 * \sa CriAtomExPcmOutputCbFunc_AT
 */
void CRIAPI criAtomEx_SetPcmOutputCallback_AT(
	CriAtomExPcmOutputCbFunc_AT cb_func, void *obj);

/*JP
 * \brief PCMデータの出力準備完了コールバック関数の登録
 * \ingroup ATOMEXLIB_AT
 * \param[in]	cb_func		PCMデータ出力準備完了のコールバック関数
 * \param[in]	obj			ユーザ指定オブジェクト
 * \par 説明:
 * PCMデータの出力準備が完了したタイミングに実行されるコールバック関数を登録します。<br>
 * PCMデータの出力準備完了について、 ::CriAtomExPcmOutputStartCbFunc_AT を参照して下さい。
 * \attention
 *  本関数の実行が ::criAtomEx_Initialize_AT 関数による初期化済みの状態で、
 *  ::criAtomEx_ExecuteMain 関数の初回実行後の場合、登録したコールバック関数は実行されません。<br>
 *  ::criAtomEx_ExecuteMain 関数の初回実行前に本関数を使用してコールバック関数を登録してください。
 * \sa CriAtomExPcmOutputStartCbFunc_AT
 */
void CRIAPI criAtomEx_SetPcmOutputStartCallback_AT(
	CriAtomExPcmOutputStartCbFunc_AT cb_func, void *obj);

/*JP
 * \brief PCMデータの出力終了コールバック関数の登録
 * \ingroup ATOMEXLIB_AT
 * \param[in]	cb_func		PCMデータの出力終了のコールバック関数
 * \param[in]	obj			ユーザ指定オブジェクト
 * \par 説明:
 * PCMデータの出力が終了したタイミングに実行されるコールバック関数を登録します。<br>
 * PCMデータの出力終了については、 ::CriAtomExPcmOutputStopCbFunc_AT を参照して下さい。
 * \sa CriAtomExPcmOutputStopCbFunc_AT
 */
void CRIAPI criAtomEx_SetPcmOutputStopCallback_AT(
	CriAtomExPcmOutputStopCbFunc_AT cb_func, void *obj);
