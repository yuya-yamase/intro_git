...
/****************************************************************************/
/* Object Name  | DCM/Dsp_SID27_TypeB/CODE                                  */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
#if ( DCM_SEC_PECULIAR_TYPE_BLUE == STD_ON )
...
#define DCM_DSP_SID27_FAILUREHIST_OFF   ((uint8)0U)                 /* failure history OFF */
#define DCM_DSP_SID27_FAILUREHIST_ON    ((uint8)1U)                 /* failure history ON*/
...
static VAR( uint8, DCM_VAR_NO_INIT )   Dcm_Dsp_SID27_u1FailureHistTmp[DCM_P_NVSEC_BLOCK_LEN];
...
VAR( uint8, DCM_VAR_SAVED_ZONE )   Dcm_u1FailureHist[DCM_P_NVSEC_BLOCK_LEN];
...
)
{
    ...
    {
        Dcm_Dsp_SID27_u1FailureHistTmp[u1_Cnt] = DCM_DSP_SID27_FAILUREHIST_OFF;
    }
    for ( u1_Cnt = (uint8)0U; u1_Cnt < u1_SecurityNum; u1_Cnt++ )
    {
        Dcm_Dsp_SID27_u1AttCntTmp[u1_Cnt] = (uint8)0U;
        /* Initialize FailureHist with failsafe value */
        Dcm_Dsp_SID27_u1FailureHistTmp[u1_Cnt] = DCM_DSP_SID27_FAILUREHIST_ON;
...
)
{
    ...
    if( ( u2_BIdxStaSec <= u2BlockIdx ) && ( u2BlockIdx <= u2_BIdxEndSec ) )
    {
        for ( u1_Cnt = (uint8)0U; u1_Cnt < u1_SecurityNum; u1_Cnt++ )
        {
            Dcm_Dsp_SID27_u1FailureHistTmp[u1_Cnt] = Dcm_u1FailureHist[u1_Cnt];
        }
    }
    return ;
}
...
)
{
    ...
    if( ( u2_BIdxStaSec <= u2BlockIdx ) && ( u2BlockIdx <= u2_BIdxEndSec ) )
    {
        for ( u1_Cnt = (uint8)0U; u1_Cnt < u1_NvSecBlockLen; u1_Cnt++ )
        {
            Dcm_u1FailureHist[u1_Cnt] = Dcm_Dsp_SID27_u1FailureHistTmp[u1_Cnt];
        }
    }
    return ;
}
...
)
{
    ...
    for( u1_Cnt = (uint8)0U; u1_Cnt < u1_SecurityNum; u1_Cnt++ )
    {
        ...
        if( u4_DelayTimerOnBoot > DCM_DSP_SID27_DTMR_NOT_START )
        {
            u1_FailureHist = Dcm_Dsp_SID27_u1FailureHistTmp[u1_Cnt];
            if ( u1_FailureHist == DCM_DSP_SID27_FAILUREHIST_OFF )
            {
                u1_TimerId = Dcm_Dsp_ConvertIdTbl[u1_Cnt].u1TimerId;
    ...
    if( u1_SecLvIndex < u1_SecurityNum )
    {
        ...
        switch( u1_RetCompareKeyFnc )
        {
            case E_OK:
                ...
                Dcm_Dsp_SID27_u1FailureHistTmp[u1_SecLvIndex] = DCM_DSP_SID27_FAILUREHIST_OFF;
                Dcm_Dsp_SID27_WriteStorage();
            ...
            case DCM_E_COMPARE_KEY_FAILED:
                ...
                Dcm_Dsp_SID27_u1FailureHistTmp[u1_SecLvIndex] = DCM_DSP_SID27_FAILUREHIST_ON;
                Dcm_Dsp_SID27_WriteStorage();
...
)
{
    ...
    if( u1_SecLvIndex < u1_SecurityNum )
    {
        ...
        Dcm_Dsp_SID27_u1MngStatus = DCM_DSP_SID27_NO_SEEDREQUEST;
        Dcm_Dsp_MsgMaker_SendNegRes( &Dcm_Dsp_Main_stMsgContext, DCM_E_INVALIDKEY );      /* NRC 0x35 */
    }
...
)
{
    ...
    if( u1SecLevelIndex < u1_SecurityNum )
    {
        ...
        else
        {
            ...
            if( Dcm_Dsp_SID27_bReadStorageCompleteFlag != (boolean)FALSE )
            {
                Dcm_Dsp_SID27_u1FailureHistTmp[u1SecLevelIndex] = DCM_DSP_SID27_FAILUREHIST_OFF;
                Dcm_Dsp_SID27_WriteStorage();
            }
        }
    }
...
#define DCM_STOP_SEC_CODE
#include <Dcm_MemMap.h>
#endif /* DCM_SEC_PECULIAR_TYPE_BLUE == STD_ON */
