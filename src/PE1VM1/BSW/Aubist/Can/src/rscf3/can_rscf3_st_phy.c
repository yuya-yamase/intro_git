/* can_rscf3_st_phy_c_m10700_v3-0-0                                         */
/****************************************************************************/
/* Protected                                                                */
...
/****************************************************************************/
/* Object Name  | CAN/RSCF3/ST/PHYSICAL/CODE                                */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
#include <can/rscf3/can_rscf3_fs.h>
#include <can/rscf3/can_rscf3_st.h>

#if (CAN_CFG_TRANSMIT == CAN_USE)
#include <can/rscf3/can_rscf3_tx.h>
...
Can_st_Stop( uint8 u1Controller )
{
    ...
    uint32                              u4TxMbUseBitMask;   /* 使用MBOXビットマスク                           */
    uint32                              u4TMTRSTSm;         /* 送信バッファ送信要求ステータスレジスタラッチ用 */
#endif /* (CAN_CFG_TRANSMIT == CAN_USE) */
    uint8                               u1Ret;              /* 戻り値                                         */
    ...
    if ( ( *(ptChReg->ptu4CmSTS) & CAN_u4CMSTS_MODEALL_MSK ) == CAN_u4CMSTS_CMODE_COMM )
    {
        ...
        u4TxMbUseBitMask = ( ptTxMbBit->u4TxIntrBit | ptTxMbBit->u4TxPollBit );
        u4TMTRSTSm       = *( ptChReg->ptu4TMTRSTSm );
        if ( ( u4TMTRSTSm & u4TxMbUseBitMask ) == CAN_u4BIT_NONE )
        {
        ...
        else
        {
            ...
            /* CFRXIEビットを下げる */
            ptChReg->ptu4CFCCk[ CAN_TRX_FIFO_0 ] = ptSetRSCANReg->u4CFCC0Conf & CAN_u4CFCCK_CFRXIE_DIS & CAN_u4CFCCK_CFE_NOUSE;
            ptChReg->ptu4CFCCk[ CAN_TRX_FIFO_1 ] = ptSetRSCANReg->u4CFCC1Conf & CAN_u4CFCCK_CFRXIE_DIS & CAN_u4CFCCK_CFE_NOUSE;

            /* 未使用送受信FIFOの設定値をリフレッシュ */
            ptChReg->ptu4CFCCk[ CAN_TRX_FIFO_2 ] = ptSetRSCANReg->u4CFCC2Conf;

            /* 送受信FIFOバッファステータスレジスタ(CFSTSk)のクリア（割り込み受信要因クリア） */
            ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_0 ] = CAN_u4CFSTSK_CLEARALL;    /* 割り込み受信用   */
            ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_1 ] = CAN_u4CFSTSK_CLEARALL;    /* ポーリング受信用 */
            ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_2 ] = CAN_u4CFSTSK_CLEARALL;    /* 未使用           */
        }
    }
    else
    {
...
Can_st_SetBaudrate( uint8 u1Controller, uint16 u2BaudRateConfigID )
{
    ...
    if ( u2BaudRateConfigID < u1BRSwtCfgTbl_ARYSIZE )
    {
        ...
        /* ボーレート設定 */
        /* チャネル通常ビットレートコンフィグレーションレジスタ(CmNCFG) */
...
Can_st_InitGlobalReg( void )
{
    ...
    *(Can_tb_stGlobalReg.ptu4RMNB)     = CAN_u4RMNB_INIT;
    ...
    for ( u1RxFifo = (uint8)0U; u1RxFifo < (uint8)CAN_NUM_FIFO_RCV; u1RxFifo++ )
    {
        ...
        /* 受信 FIFO バッファCAN FDステータスレジスタ(RFFDSTSx)         */
    ...
    for ( u1Controller = (uint8)0U; u1Controller < u1CtrlNum; u1Controller++ )
    {
        ...
        if ( Can_tb_stCtrlInfo[ u1Controller ].u1CtrlAct == (uint8)CAN_TRUE )
        {
            ...
            /* 送受信FIFO バッファCAN FDコンフィグレーション／ステータスレジスタ(CFFDCSTSk) */
            /* 送受信FIFO バッファアクセスデータフィールドdレジスタk(CFDFd_k)               */
            /* → 本設計としては、送受信FIFOバッファは未使用または受信モードとする。        */
            /*    受信モードでは書き込み禁止。                                              */
        }
    }
...
Can_st_InitChannelReg( uint8 u1Controller )
{
    ...
    *(ptChReg->ptu4CmFDCFG) = CAN_u4CMFDCFG_INIT;
    ...
    for ( u1MBox = (uint8)0U; u1MBox < (uint8)CAN_NUM_BUF_SND_PER_CH; u1MBox++ )
    {
        ...
        for ( u1Data = (uint8)0U; u1Data < (uint8)CAN_TB_MB_DATA_ARYSIZE; u1Data++ )
        {
    ...
    /* 送信バッファ割り込みイネーブルコンフィグレーションレジスタ(TMIECm) */
    *(ptChReg->ptu4TMIECm)   = CAN_u4TMIECM_INIT;

    /*------------------------------------*/
    /* 送信バッファステータス関連レジスタ */
    /*------------------------------------*/
    /* 送信バッファ送信要求ステータスレジスタ(TMTRSTSm)          */
    /* 送信バッファ送信アボート要求ステータスレジスタ(TMTARSTSm) */
    /* 送信バッファ送信完了ステータスレジスタ(TMTCSTSm)          */
    /* 送信バッファ送信アボートステータスレジスタ(TMTASTSm)      */
    /* → リードオンリーのため初期化対象なし                     */
    ...
    *(ptChReg->ptu4THLCCm)   = CAN_u4THLCCM_INIT;
    ...
    /* 送信履歴アクセスレジスタ0m(THLACC0m)  */
    /* 送信履歴アクセスレジスタ1m(THLACC1m)  */
    /* → リードオンリーのため初期化対象なし */
}
    ...
    *(Can_tb_stGlobalReg.ptu4GCTR) = CAN_u4GCTR_INIT | CAN_u4GCTR_TSRST_RESET | CAN_u4GCTR_GMDC_RESET_SET;
    ...
    for ( u1Controller = (uint8)0U; u1Controller < u1CtrlNum; u1Controller++ )
    {
        ...
        if ( Can_tb_stCtrlInfo[ u1Controller ].u1CtrlAct == (uint8)CAN_TRUE )
        {
            ...
            ptChReg->ptu4CFCCk[ CAN_TRX_FIFO_0 ] = ptSetRSCANReg->u4CFCC0Conf & CAN_u4CFCCK_CFRXIE_DIS & CAN_u4CFCCK_CFE_NOUSE; /* 割り込み受信用設定   */
            ptChReg->ptu4CFCCk[ CAN_TRX_FIFO_1 ] = ptSetRSCANReg->u4CFCC1Conf & CAN_u4CFCCK_CFRXIE_DIS & CAN_u4CFCCK_CFE_NOUSE; /* ポーリング受信用設定 */
            ptChReg->ptu4CFCCk[ CAN_TRX_FIFO_2 ] = ptSetRSCANReg->u4CFCC2Conf & CAN_u4CFCCK_CFRXIE_DIS & CAN_u4CFCCK_CFE_NOUSE; /* 未使用設定           */
        }
    }
...
Can_st_SetChannelReg( uint8 u1Controller )
{
    ...
    *(ptChReg->ptu4CmCTR)    = ptSetRSCANReg->u4CmCTRConf | CAN_u4CMCTR_CHMDC_RESET_SET;
    ...
    /* 送信バッファ割り込みイネーブルコンフィグレーションレジスタ(TMIECm)：割り込み禁止 */
    *(ptChReg->ptu4TMIECm)   = CAN_u4TMIECM_INIT;
...
Can_st_StartChannel( uint8 u1Controller )
{
    ...
    {   /* チャネルリセットモードの場合 */
        ...
        *(Can_tb_stGlobalReg.ptu4CDTCT) = CAN_u4CDTCT_INIT;
        ...
        ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_0 ] = CAN_u4CFSTSK_CLEARALL;          /* 割り込み受信用設定   */
        ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_1 ] = CAN_u4CFSTSK_CLEARALL;          /* ポーリング受信用設定 */
        ptChReg->ptu4CFSTSk[ CAN_TRX_FIFO_2 ] = CAN_u4CFSTSK_CLEARALL;          /* 未使用設定           */

        /* 送受信FIFOバッファコンフィグレーションレジスタ(CFCCk) */
        ...
        /* 送信完了割り込み許可設定 */
        /* - 送信バッファ割り込みイネーブルコンフィグレーションレジスタ(TMIECm) */
        *(ptChReg->ptu4TMIECm)  = Can_tb_stTxMbBit[ u1Controller ].u4TxIntrBit;
