/* Dem_DataCtl_FilteredRecord_RtvlOff_c(v5-10-0)                            */
/****************************************************************************/
/* Protected                                                                */
...
/****************************************************************************/
/* Object Name  | Dem/DataCtl_FilteredRecord_RtvlOff/CODE                   */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
)
{
    ...
    obdRecordNumberIndex = FilteredRecordSearchInfoPtr->ObdRecordNumberIndex;
    recordNumberIndex = FilteredRecordSearchInfoPtr->RecordNumberIndex;
    tsffListIndex = FilteredRecordSearchInfoPtr->TSFFListIndex;
    tsffRecIndex = FilteredRecordSearchInfoPtr->TSFFRecIndex;

    /* Gets filtered record information for non-OBD corresponding to FreezeFrameList Index. */
    retVal = Dem_Data_GetFilteredRecordInfo( EventStrgIndex, &faultRecord, &obdRecordNumberIndex, &recordNumberIndex, &tsffListIndex, &tsffRecIndex, DTCValuePtr, RecordNumberPtr );    /* [GUD]eventStrgIndex */

    FilteredRecordSearchInfoPtr->ObdRecordNumberIndex = obdRecordNumberIndex;
    FilteredRecordSearchInfoPtr->RecordNumberIndex = recordNumberIndex;
    FilteredRecordSearchInfoPtr->TSFFListIndex = tsffListIndex;
    FilteredRecordSearchInfoPtr->TSFFRecIndex = tsffRecIndex;
...
)
{
    ...
    VAR( boolean, AUTOMATIC ) retCheckOutputMultiCylObdFFD;
...
#if ( DEM_MISFIRE_OUTPUT_UDSOBDFFD_MULTIPLE_FAILED_CYL_SUPPORT == STD_ON )      /*  [FuncSw]    */
    retCheckOutputMultiCylObdFFD    =   (boolean)FALSE;
#endif  /* ( DEM_MISFIRE_OUTPUT_UDSOBDFFD_MULTIPLE_FAILED_CYL_SUPPORT == STD_ON )               */
    ...
    for( recordNumberIndex = *ObdRecordNumberIndexPtr; recordNumberIndex < obdFFRClassPerDTCMaxNum; recordNumberIndex++ )   /* [GUD:for] recordNumberIndex */
    {
        ...
        freezeFrameRecordClassIndex = FreezeFrameRecNumClassPtr->DemFreezeFrameRecordClassRef[recordNumberIndex];           /* [GUD] recordNumberIndex *//* [ARYCHK] DEM_FF_RECORD_CLASS_REF_MAX_NUM / 1 / recordNumberIndex */
        ...
        if( freezeFrameRecordClassIndex < ffrRecordClassConfigureNum )                                                      /* [GUD:if] freezeFrameRecordClassIndex */
        {
            ...
            if( FaultRecordPtr->ObdRecordNumberIndex[recordNumberIndex] < obdFFDRecordNum )                                 /* [GUD] recordNumberIndex *//* [ARYCHK] DEM_OBD_FFR_CLASS_PER_DTC_MAX_NUM / 1 / recordNumberIndex */
            {
                /* The index is valid. */
                ...
                *RecordNumberPtr    =   freezeFrameRecordNumber;
                ...
                if( misfireEventKind == (boolean)TRUE )
                {
                    ...
                    {
#if ( DEM_MISFIRE_OUTPUT_UDSOBDFFD_MULTIPLE_FAILED_CYL_SUPPORT == STD_ON )      /*  [FuncSw]    */
                        retCheckOutputMultiCylObdFFD    =   (boolean)TRUE;
#endif  /* ( DEM_MISFIRE_OUTPUT_UDSOBDFFD_MULTIPLE_FAILED_CYL_SUPPORT == STD_ON )               */
...
#if ( DEM_MISFIRE_OUTPUT_UDSOBDFFD_MULTIPLE_FAILED_CYL_SUPPORT == STD_ON )      /*  [FuncSw]    */
    if( retCheckOutputMultiCylObdFFD == (boolean)TRUE )
    {
...
)
{
    ...
    if ( dtcAttributePtr != NULL_PTR )                                              /* [GUD:if]dtcAttributePtr */
    {
        ...
        tsFFRecClassRef = dtcAttributePtr->DemTimeSeriesFreezeFrameRecordClassRef[TSFFListIndex];    /* [GUD]dtcAttributePtr *//* [GUDCHK:CALLER]TSFFListIndex *//* [ARYCHK] DEM_TSFF_RECORD_CLASS_NUM_PER_DTC_MAX_NUM / 1 / TSFFListIndex */
...
)
{
    ...
    for( recordNumberIndex = *RecordNumberIndexPtr; recordNumberIndex < nonOBDFFRClassPerDTCMaxNum; recordNumberIndex++ )   /* [GUD:for] recordNumberIndex */
    {
        ...
        if( freezeFrameRecordClassIndex < ffrRecordClassConfigureNum )                                                      /* [GUD:if] freezeFrameRecordClassIndex */
        {
            ...
            /* pointed by the auto variable "recordNumberIndex" is invalid or not. */
            if( FaultRecordPtr->RecordNumberIndex[recordNumberIndex] < nonObdFFDRecordNum )                                 /* [GUD] recordNumberIndex *//* [ARYCHK] DEM_NONOBD_FFR_CLASS_PER_DTC_MAX_NUM / 1 / recordNumberIndex */
            {
                /* The index is valid. */
