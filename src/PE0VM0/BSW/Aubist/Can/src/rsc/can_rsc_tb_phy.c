/* can_rsc_tb_phy_c_m10700_v3-0-0                                           */
/****************************************************************************/
/* Protected                                                                */
...
/****************************************************************************/
/* Object Name  | CAN/RSC/TB/PHYSICAL/CODE                                  */
/*--------------------------------------------------------------------------*/
/* Notes        |                                                           */
/****************************************************************************/
...
/* CFIM(送受信FIFO割込み要因) */
#define CAN_TB_u4CFCCk_CFIM_CFIGCV          ( 0x00000000UL )              /* CFICGVに指定したタイミングで割り込み(リセット初期値) */
#define CAN_TB_u4CFCCk_CFIM_ON_1MSG         ( 0x00001000UL )              /* 1メッセージ受信する毎に割り込み                      */
...
#define CAN_TB_u4CFCCk_CFDC_INT             ( CAN_TB_u4CFCCk_CFDC_4 )     /* 割り込み受信用FIFO段数設定値(4段固定)                */
#define CAN_TB_u4CFCCk_CFDC_POL             ( CAN_TB_u4CFCCk_CFDC_48 )    /* ポーリング受信用FIFO段数設定値(48段固定)             */
...
/* 用途別設定値 */
#define CAN_TB_u4CFCCk_INTERRUPT            ( CAN_TB_u4CFCCk_CFM_RX | CAN_TB_u4CFCCk_CFIM_ON_1MSG | CAN_TB_u4CFCCk_CFDC_INT | CAN_TB_u4CFCCk_CFTXIE_DIS | CAN_TB_u4CFCCk_CFRXIE_ENA | CAN_TB_u4CFCCk_CFE_USE )   /* 割り込み受信動作用設定値   */
#define CAN_TB_u4CFCCk_POLLING              ( CAN_TB_u4CFCCk_CFM_RX | CAN_TB_u4CFCCk_CFIM_CFIGCV  | CAN_TB_u4CFCCk_CFDC_POL | CAN_TB_u4CFCCk_CFTXIE_DIS | CAN_TB_u4CFCCk_CFRXIE_DIS | CAN_TB_u4CFCCk_CFE_USE )   /* ポーリング受信動作用設定値 */
#define CAN_TB_u4CFCCk_NOUSE                ( CAN_TB_u4CFCCk_CFM_RX | CAN_TB_u4CFCCk_CFIM_CFIGCV  | CAN_TB_u4CFCCk_CFDC_0   | CAN_TB_u4CFCCk_CFTXIE_DIS | CAN_TB_u4CFCCk_CFRXIE_DIS | CAN_TB_u4CFCCk_CFE_NOUSE ) /* 未使用設定値               */

/* CFCC0レジスタ設定値 */
#define CAN_TB_u4CFCC0_CONF( ch )           ( (CAN_CFG_RX_PROCESSING_##ch == CAN_INTERRUPT) ? CAN_TB_u4CFCCk_INTERRUPT : CAN_TB_u4CFCCk_NOUSE )
/* CFCC1レジスタ設定値 */
#define CAN_TB_u4CFCC1_CONF( ch )           ( (CAN_TB_RXPOLLING( ch )    == CAN_USE)        ? CAN_TB_u4CFCCk_POLLING   : CAN_TB_u4CFCCk_NOUSE )
/* CFCC2レジスタ設定値 */
...
#define CAN_TB_u4GCFG_CONF \
    ( CAN_TB_u4GCFG_BASE | CAN_TB_u4GCFG_DCS_CONF | CAN_TB_u4GCFG_TPRI_CONF )
...
/*   - u4CmCFGConf       チャネルコンフィグレーションレジスタ設定値                   */
/*   - u4CmCTRConf       チャネル制御レジスタ設定値                                   */
/*   - u4CFSTS0Conf      不正割り込み時用割り込みフラグクリア値                       */
/*------------------------------------------------------------------------------------*/
